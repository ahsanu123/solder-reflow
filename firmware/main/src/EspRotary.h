...
enum class rotary_direction { undefined = 0, right = 1, left = 255 };
...
public:
  ESPRotary();
  ESPRotary(byte pin1, byte pin2, byte steps_per_click = 1,
            int lower_bound = INT16_MIN, int upper_bound = INT16_MAX,
            int inital_pos = 0, int increment = 1);

  void begin(byte pin1, byte pin2, byte steps_per_click = 1,
             int lower_bound = INT16_MIN, int upper_bound = INT16_MAX,
             int inital_pos = 0, int increment = 1);
  ...
  std::string directionToString(rotary_direction dir) const;
