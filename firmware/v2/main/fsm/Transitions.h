...
public:
  Transition();
  Transition(State *from, State *to, int event_id,
             CallbackFunction on_run = NULL, std::string name = "",
             GuardCondition guard = NULL);

  void setup(State *from, State *to, int event_id,
             CallbackFunction on_run = NULL, std::string name = "",
             GuardCondition guard = NULL);
...
public:
  TimedTransition();
  TimedTransition(State *from, State *to, int interval,
                  CallbackFunction on_run = NULL, std::string name = "",
                  GuardCondition guard = NULL);

  void setup(State *from, State *to, int interval,
             CallbackFunction on_run = NULL, std::string name = "",
             GuardCondition guard = NULL);
